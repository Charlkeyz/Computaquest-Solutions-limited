...
const Header = () => {
  ...
  const headerBgClass =
    isHomePage ?
      scrolled ? "bg-white shadow-md text-black"
  ...
  return (
    <>
      <header
        className={`flex flex-col w-full z-[100] transition-all duration-300 fixed top-0 ${headerBgClass}`}>
        {/* Desktop Header */}
        <div className="hidden slg:flex items-center justify-between w-full py-6 max-w-[1440px] px-8 mx-auto">
          {/* Left: Logo */}
          <Link
            href="/"
              ...
              <Link
                ...
                className={`text-sm font-medium transition-all relative group ${
                  pathname === link.href ?
                    ...
                    : "text-black"
                  : isHomePage && !scrolled ? "text-white/80 hover:text-white"
                  : "text-gray-600 hover:text-black"
                }`}>
                {link.text}
                <span
                  className={`h-[2px] inline-block absolute left-0 -bottom-1 transition-all ease duration-300 ${
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <Menu as="div" className="relative inline-block text-left">
                <Menu.Button
                  className={`flex items-center cursor-pointer group outline-none focus:ring-0 transition ${
                    isHomePage && !scrolled ?
                      "text-white hover:text-white/80"
                    : "text-gray-600 hover:text-black"
                  }`}>
                  ...
                  <Menu.Items className="absolute right-0 mt-2 w-52 origin-top-right bg-white border border-gray-200 rounded-2xl shadow-xl p-1.5 z-[110] outline-none">
                    <div className="px-3 py-2 mb-1 border-b border-gray-100">
                      ...
                      <p className="text-sm font-bold text-gray-900 truncate">
                        {wc_customer_info?.first_name || "Guest"}
                      </p>
                    </div>
              ...
              <div
                ...
                {totalItems > 0 && (
                  <span
                    className={`absolute -top-1.5 -right-1.5 size-4 text-[9px] font-black flex items-center justify-center rounded-full ${
                      isHomePage && !scrolled ?
                        "bg-white text-black"
                      : "bg-black text-white"
                    }`}>
              ...
              <Link
                href="/contact-us"
                className={`text-xs font-bold px-6 py-2.5 rounded-full transition-colors shadow-sm ${
                  isHomePage && !scrolled ?
        ...
        {/* Accent line - only show when not transparent */}
        {!isHomePage && (
